    ...
    public sealed class PCPShaderAnalyzer : PCPModuleBase
    {
        ...
        public override int FindingCount
        {
            get
            {
                ...
                {
                    if (_results[i].GetSeverity() != PCPSeverity.Info)
                        count++;
                }
                return count;
            }
        ...
        protected override async Task DoScanAsync(PCPScanContext context, CancellationToken ct)
        {
            ...
            // ----------------------------------------------------------
            // Phase 4: Analyse each shader (main-thread metadata + parallel I/O)
            // ----------------------------------------------------------
            int totalShaders = shaderPaths.Count;
            ReportProgress(0.3f, $"Analysing {totalShaders} shaders...");

            // Step 1: Main thread — load Shader objects and check cache.
            var shaderEntries = new List<PCPShaderEntry>(totalShaders);
            var needsSourceParse = new List<(int index, string path, string fullPath)>();
            ...
            foreach (string shaderPath in shaderPaths)
            {
                ...
                // File size — try cache first.
                long cachedSize = context.Cache.GetFileSize(shaderPath);
                if (cachedSize > 0 && !context.Cache.IsStale(shaderPath))
                {
                ...
                else
                {
                    ...
                    {
                        try { entry.sizeBytes = new FileInfo(fullSizePath).Length; }
                        catch { entry.sizeBytes = 0; }
                        context.Cache.SetFileSize(shaderPath, entry.sizeBytes);
                    }
                }

                if (isShaderFile)
                {
                    // Try cached shader parse results.
                    bool usedCache = false;
                    if (!context.Cache.IsStale(shaderPath))
                    {
                        string cachedKeywords = context.Cache.GetMetadata(shaderPath, "shader.keywords");
                        string cachedPassCount = context.Cache.GetMetadata(shaderPath, "shader.passCount");
                        string cachedVariants = context.Cache.GetMetadata(shaderPath, "shader.variants");
                    ...
                    if (!usedCache)
                    {
                        ...
                        if (File.Exists(fullPath))
                            needsSourceParse.Add((shaderIndex, shaderPath, fullPath));
                    }
                }
                else
                {
            ...
            // Step 2: Background — read and parse stale shader sources in parallel.
            if (needsSourceParse.Count > 0)
            {
                ReportProgress(0.6f, $"Parsing {needsSourceParse.Count} shader sources...");

                var parseResults = new ConcurrentDictionary<int, string>();

                await PCPThreading.ParallelForEachAsync(needsSourceParse, (item, token) =>
                {
                    token.ThrowIfCancellationRequested();
                    try
                    {
                        string source = File.ReadAllText(item.fullPath);
                        parseResults[item.index] = source;
                    }
                    catch (Exception)
                    {
                        // Ignore read errors.
                ...
                // Step 3: Apply parse results and update cache (main thread).
                foreach (var (idx, path, _) in needsSourceParse)
                {
                    if (!parseResults.TryGetValue(idx, out string sourceText))
                        continue;

                    var entry = shaderEntries[idx];
                    if (entry == null) continue;

                    ParseShaderSource(sourceText, entry);

                    context.Cache.SetMetadata(path, "shader.keywords",
                        entry.keywords != null ? string.Join(",", entry.keywords) : "");
                    context.Cache.SetMetadata(path, "shader.passCount",
                        entry.passCount.ToString());
                    context.Cache.SetMetadata(path, "shader.variants",
                        entry.estimatedVariants.ToString());

                    // Pipeline mismatch may change after source parsing sets targetPipeline.
                    if (PCPSettings.instance.shaderAnalyzerCheckPipeline &&
                        entry.targetPipeline != PCPRenderPipeline.Custom &&
                        projectPipeline != PCPRenderPipeline.Custom &&
                        entry.targetPipeline != projectPipeline)
